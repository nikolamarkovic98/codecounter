...
document
    .querySelector("#github-calculate")
    .addEventListener("click", calculateRepo);
document
    .querySelector("#select-calculate")
    .addEventListener("click", calculate);

fileLoader.addEventListener("click", (e) => {
    if (filesAdded) e.preventDefault();
});
...
document.body.addEventListener("click", (e) => {
    ...
    if (dataAttr == "exclude") {
        if (e.target.innerHTML == "Exclude") {
            if (excludeMode) {
                document.getElementById("exclude-file-list").style.display =
                    "none";
            } else {
                document.getElementById("exclude-file-list").style.display =
                    "block";
        ...
        if (excludeMode) excludeMode = !excludeMode;
    ...
    if (dataAttr == "file-filter") {
        if (e.target.innerHTML == "Filter") {
            if (filterMode) {
                document.querySelector("#file-filter-list").style.display =
                    "none";
            } else {
                document.querySelector("#file-filter-list").style.display =
                    "block";
        ...
        if (filterMode) filterMode = !filterMode;
