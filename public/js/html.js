...
calculateResult = async (type) => {
    if (type != "") {
        ...
        for (; index < results.length; index++)
            if (results[index].type == type) break;
        setResult(
            results[index].lines,
            results[index].words,
            results[index].chars,
            results[index].num_of_files
        );
    } else {
...
displayResult = (mode) => {
    ...
    let lis =
        '<li data-value="" data-id="file-filter" class="filter-item">All Files</li>';
    for (let i = 1; i < results.length; i++)
...
const endAnimation = () => {
    ...
    animationIndex = 0;
    breaker = true;
    for (let i = 0; i < 3; i++) dots[i].style.opacity = 0;
};
