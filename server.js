...
const getName = (str) => {
    ...
    if (!str.includes("/")) return "Unknown";
    let mario = "";
    let i = str.length - 1;
    if (str[i] == "/") str.length--;
    while (str[i] != "/") {
...
const repoExist = async (repoName) => {
    return new Promise(async (resolve, reject) => {
        ...
        for await (const dirent of dirs) {
            if (dirent.name.includes(repoName)) match = true;
        }
        resolve(match);
    });
};
    ...
    return new Promise(async (resolve, reject) => {
        ...
        const msg = await new Promise(async (resolve, reject) => {
            ...
            for (let index = 0; index < files.length; index++) {
                const [lines, words, chars, type] = await readFile(
                    files[index]
                );
                num_of_lines += lines;
                num_of_words += words;
                ...
                if (files.length - 1 == index) resolve("Success");
            }
        });
